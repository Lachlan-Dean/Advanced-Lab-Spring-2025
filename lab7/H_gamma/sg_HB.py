...
just_HT = np.abs(HAT_data[480:550])
...
std_dark = np.std(dark_cur, ddof=1)

# Calculating quad
HA_mean = HA_mean - dark_curmean
diff_curHA = np.abs(HA_cur) - dark_curmean
HA_err = np.abs(just_HTcur) * np.sqrt(std_dark**2 + ((np.abs(just_HTcur) - dark_curmean)*HA_std/(HA_mean-dark_curmean))**2)

# Fitter function
def func(x, p):
    z = p[0] * np.exp(-((x - p[1])**2)/(2*p[2])) + p[3]
    return z
