...
HBT_data = np.genfromtxt('H_epsilon_transition.TXT')
HB_lambda = HBT_data[:,0]
HB_cur = HBT_data[:,1]

bf_data = np.genfromtxt('flicker5to2_data.dat')
HBF_cur = bf_data[:,1]
...
just_HT = np.abs(HBT_data[500:620])
just_HTcur = just_HT[:,1]
just_HTlam = just_HT[:,0]
just_DT = np.abs(HBT_data[300:380])
