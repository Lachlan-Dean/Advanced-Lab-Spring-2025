...
just_HT = np.abs(HAT_data[450:510])
...
HA_err = just_HTcur * np.sqrt(std_dark**2 + ((np.abs(just_HTcur) - dark_curmean)*HA_std/(HA_mean-dark_curmean))**2)
...
ax.errorbar(just_HTlam,just_HTcur, yerr=HA_err, fmt='|', capsize=5, color='green')
plt.show()
plt.close()
