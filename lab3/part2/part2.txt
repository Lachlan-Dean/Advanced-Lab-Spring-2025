...
def func(x, p):
    z = p[0] * np.exp(-(x - p[1]) * (x - p[1])/2./p[2])
    return z
...
x, y, sig = np.genfromtxt("part2_data-1.txt", unpack=True)
...
xfit = np.linspace(-50, 150, 1500)
yfit = pfinal[0] * np.exp(-(xfit - pfinal[1]) * (xfit - pfinal[1])/2./pfinal[2])
plt.figure()
plt.subplot(211)
