...
data = np.genfromtxt('Ne_WM_1scan_CL.TXT')
I = np.abs(data[:,1])
lam = data[:,0]
...
# Fitter function
def func(x, p):
    z= (p[0] * np.exp(-(x - p[1]) * (x - p[1])/2./p[2])) + (p[3] * np.exp(-(x - p[4]) * (x - p[4])/2./p[5])) + (p[6] * np.exp(-(x - p[7]) * (x - p[7])/2./p[8])) + p[9]
...
p0 = [.5,607.136,.0001,1.2,607.152,.0001,.5, 607.168,.0001,.19]
...
left_cen = ufloat(pfinal[1], 2.384029599863694e-06)
right_cen = ufloat(pfinal[7], 1.0345176358947937e-06)
cen = ufloat(pfinal[4], 2.1645303006621943e-06)

left_ch = left_cen - cen
right_ch = cen - right_cen
h = 6.626 * (10**(-34))
c = 3 * (10**8) 
hc = 1240
...
#ax[0].scatter(lam, I, color='green')
